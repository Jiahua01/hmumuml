...
void bdtscorefit(TString mode = "two_jet", int button = 0)
{
    ...
    TH1D *h_diboson = new TH1D(Form("%s", LG[0].Data()), Form("%s", LG[0].Data()),nbins, bin); 
    TH1D *h_top = new TH1D(Form("%s", LG[1].Data()), Form("%s", LG[1].Data()),nbins, bin); 
    TH1D *h_ewk = new TH1D(Form("%s", LG[2].Data()), Form("%s", LG[2].Data()),nbins, bin); 
    TH1D *h_dy = new TH1D(Form("%s", LG[3].Data()), Form("%s", LG[3].Data()),nbins, bin); 
    TH1D *h_data = new TH1D(Form("%s", LG[4].Data()), Form("%s", LG[4].Data()),nbins, bin); 
    TH1D *h_vbf = new TH1D(Form("%s", LG[5].Data()), Form("%s", LG[5].Data()),nbins, bin); 
    ...
    for (int typei = 0; typei < stypes; typei++)
    {
      ...
      //  t->SetBranchAddress("trg_single_mu24", &tag);
        ...
        for (Long64_t i = 0; i < nentries; i++)
        {
            ...
            if ((hmass>110 && hmass<115)||(hmass>135 && hmass<150)) {//hmass>115 && hmass<135//(hmass>110 && hmass<115)||(hmass>135 && hmass<150)
    ...
    TString hbkroot = "bdtscore_SB.root";
