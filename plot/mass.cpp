...
void mass(TString mode = "zero_jet", int button = 0)
{
    ...
    TString LG[7] = { "Diboson", "Top", "EWK ZG", "DY","Data","VBF(x20)"};
    double Y_axis[3] = {0, 0.2, 100};

    TString x_name = "H_mass";
    TString str_dir = "/eos/user/j/jiahua/vbfhmm/MC_guo/outputs/two_jet";
    TString stype[7][4] = {{"diboson","0"},{"top", "0"}, {"ewk", "0"},{"dy","0"}};//, {"mc_med", "0"}};//, "sig"};
    TString ltype[][4] = {{"data","0"}, {"vbf", "0"}};//, {"ggH", "0"}, {"VBF", "0"}, {"WminusH", "WplusH", "ZH", "0"}, {"ttH", "0"}};

    int X_axis[]={80,70,150}, bins;
    int stypes = 4, ltypes = 2;
  ...
  //  Bool_t tag;
    ...
    for (int typei = 0; typei < stypes; typei++)
    {
        ...
        {
        cout << "\n\tLoading file" << Form("%s/%s.root", str_dir.Data(), stype[typei][len].Data()) << "!......" << endl;
        TFile *f = new TFile(Form("%s/%s.root", str_dir.Data(), stype[typei][len].Data()));
      ...
      //  t->SetBranchAddress("trg_single_mu24", &tag);
        ...
        {
            t->GetEntry(i);
            if (hmass<110 | hmass>150) continue;
        ...
        cout << "\n\t" << h1[typei]->Integral() << " items are in it!!";
    }
    ...
    for (int typei = 0; typei < ltypes; typei++)
    {
        ...
        {
            cout << "\n\tLoading file" << Form("%s/%s.root", str_dir.Data(), ltype[typei][len].Data()) << "!......" << endl;

            TFile *f = new TFile(Form("%s/%s.root", str_dir.Data(), ltype[typei][len].Data()));
        ...
        cout << "\tFinished loading File" << endl;
        cout << "\n\t" << h2[typei]->Integral() << " items are in it!!";
    }
 ...
 /*   for (int typei=0; typei<stypes; typei++){
  ...
  //  if (max == mc_h->GetBinContent(mc_h->GetMaximumBin()))
   ...
   // text->DrawLatex(0.5, 0.85, Form("#chi^{2}/n: %.4f", kai2/good_bin));
    ...
    data_h->GetXaxis()->SetTitle(Form("%s: M_{#mu#mu}[GeV]", mode.Data()));
    can->Update();
