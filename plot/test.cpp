...
void test(TString mode = "two_jet", int button = 0)
{
    ...
    TString LG[10] = {"Triboson","Diboson", "Top", "EWK ZG", "DY","Data","VBF*20","ggH"};
    double Y_axis[3] = {0, 0.2, 100};

    TString x_name = "log_Hpt";
    TString str_dir = "/eos/user/j/jiahua/vbfhmm/MC_guo";
        ...
        while (ltype[typei][len] != "0")
        {
          ...
          //  t->SetBranchAddress("trg_single_mu24", &tag);
            ...
            for (Long64_t i = 0; i < nentries; i++)
            {
                ...
                data = static_cast<double_t>(temp);
                if ((typei == 0) && (hmass < 135 && hmass > 115))
                {
                    h2[typei]->Fill(data);
                    data_h->Fill(data);
                    num[typei + stypes] += 1;
